...
`<mastodon.orgのあるディレクトリ>/.attach`（ID分割構成）を自動的に使用する
（追加の引数は不要）。変換先は `<Org Directory>/mastodon/{YYYY}/images/` で、
ID分割なしのフラット構成にコピーする（FR-009, FR-011）。

## 前提条件

- 変換元ファイルは変更・削除されない（FR-007）。
- 変換元の添付ファイル（`.attach` 配下、ID分割構成）は変更・削除されない（FR-011）。
